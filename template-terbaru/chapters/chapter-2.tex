...
\begin{table}[H]
    \centering
    \caption{Literasi Penelitian Terdahulu (Bagian 1)}
...
\begin{table}[H]
    \centering
    \caption{Literasi Penelitian Terdahulu (Bagian 2)}
...
\textit{Self-harm} atau perilaku menyakiti diri sendiri merupakan tindakan yang dilakukan seseorang untuk melukai tubuhnya secara sengaja, baik dengan maupun tanpa niat untuk mengakhiri hidup. WHO (Organisasi Kesehatan Dunia) mendefinisikan \textit{self-harm} sebagai \textit{ ``intentional self-inflicted injury, with or without suicidal intent''} \cite{WHO2014PreventingSuicide}. Dalam ranah psikiatri, DSM-5 (\textit{Diagnostic and Statistical Manual of Mental Disorders} edisi kelima) membedakan antara NSSI (\textit{non-suicidal self-injury}), yaitu perilaku menyakiti diri tanpa niat bunuh diri yang setidaknya terjadi selama lima hari dalam setahun, dengan \textit{suicidal behavior} yang berorientasi pada keinginan mengakhiri hidup \cite{APA2022_DSM5TR}. \textit{Self-harm} banyak dikaitkan dengan kondisi psikologis seperti depresi, kecemasan, dan kesulitan regulasi emosi.
...
ELECTRA (\textit{Efficiently Learning an Encoder that Classifies Token Replacements Accurately}) merupakan model pra-latih berbasis arsitektur Transformer yang diperkenalkan sebagai alternatif yang lebih efisien dibandingkan pendekatan MLM (\textit{Masked Language Modeling}) seperti pada BERT. Berbeda dengan MLM yang hanya mempelajari representasi dari token yang di-\textit{masking}, ELECTRA menggunakan pendekatan diskriminatif yang disebut RTD (\textit{Replaced Token Detection}), di mana model dilatih untuk membedakan apakah suatu token dalam urutan merupakan token asli atau hasil penggantian oleh model \textit{generator} \cite{clark2020electra}.
...
Proses pelatihan ELECTRA dimodelkan melalui fungsi RTD (\textit{Replaced Token Detection}) \textit{loss} sebagai berikut: 
\\

\begin{equationcaptioned}[eq:rtd-loss]{
\mathcal{L}_{\text{RTD}} = - \sum_{i=1}^{n} \left[ y_i \log D(x_i) + (1 - y_i)\log \left(1 - D(x_i)\right) \right]
}{RTD (\textit{Replaced Token Detection}) \textit{Loss} pada ELECTRA}
\end{equationcaptioned}
...
Keduanya penting karena kesalahan FN (\textit{False Negative}) pada deteksi 
